	...
	(function (){
		...
		if(page.readyState){
			...
			if (zURL.get('pay') == 'done'&&zURL.get('area')) {
			    zHttp.syrequest({actid:zURL.get('actid'),roleid : zURL.get('roleid'),area : zURL.get('area'),partition: zURL.get('partition')},true);
	...
	OpenVipRequest = {
		...
		**/
		...
		init : function () {
			...
			//如果有大区就选择大区才跳转会员支付
			if (this.area) {
				this.page.require('AreaSvrSelector', function(){
		            svr = new qv.zero.AreaSvrSelector({ 
		                name : gameName,
		                serverList : me.key,
		                send : function (args) {
							zHttp.syrequest({actid : me.actid,roleid : args.roleid,area : args.area,partition: args.partition},function (json) {
								returnHref = returnHref + '&roleid=' + args.roleid + '&area=' + args.area + '&partition=' + args.partition;
								me.openVip(json,returnHref);
							});
						}
		            });
			        svr.show();
		...
		openVip : function (json,returnHref) {
			var me = this;
