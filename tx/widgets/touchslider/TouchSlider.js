;(function($){
    ...
    $.fn.touchSlider = function(option){
        ...
        function animateItem(flag, isauto){
            isAnimate = true;
            var percent = Math.abs(step)/distance,
                animTarget = (percent>opts.slidePercent || isauto) ? '0px' : distance*flag+'px',
            ...
            $items.eq(curItemIndex+flag).css({'z-index':1000});
            $items.eq(curItemIndex+flag).animate(animProperty,250,'ease-in',function(){
                if(percent>opts.slidePercent || isauto){
